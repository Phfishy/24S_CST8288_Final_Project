...
public class FoodItemDao {
  ...
  public FoodItem createFoodItem(FoodItem item) throws SQLException {
    String sql =
        "INSERT INTO food_item (retailer_id, name, description, quantity, price, discounted_price, expiration_date, is_surplus, is_for_donation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  public FoodItem getFoodItemById(long id) throws SQLException {
    String sql = "SELECT * FROM food_item WHERE id = ? and status != 5";
    try (Connection conn = DatabaseConnection.getConnection();
  ...
  public boolean updateFoodItem(FoodItem item) throws SQLException {
    String sql =
        "UPDATE food_item SET retailer_id = ?, name = ?, description = ?, quantity = ?, price = ?, discounted_price = ?, expiration_date = ?, is_surplus = ?, is_for_donation = ? WHERE id = ?";
    try (Connection conn = DatabaseConnection.getConnection();
        PreparedStatement pstmt = conn.prepareStatement(sql)) {
      pstmt.setLong(1, item.getRetailerId());
      pstmt.setString(2, item.getName());
  ...
  public boolean deleteFoodItem(long id) throws SQLException {
    String sql = "UPDATE food_item SET status = 5 WHERE id = ?";
    try (Connection conn = DatabaseConnection.getConnection();
        PreparedStatement pstmt = conn.prepareStatement(sql)) {
      pstmt.setLong(1, id);
      int affectedRows = pstmt.executeUpdate();
  ...
  public List<FoodItem> getFoodItemsByRetailerId(long retailerId) throws SQLException {
    String sql = "SELECT * FROM food_item WHERE retailer_id = ? and status != 5";
  ...
  public List<FoodItem> getFoodItemsByDonationStatus(boolean isForDonation) throws SQLException {
    String sql = "SELECT * FROM food_item WHERE is_for_donation = ? and status != 5";
    List<FoodItem> items = new ArrayList<>();
    try (Connection conn = DatabaseConnection.getConnection();
        PreparedStatement pstmt = conn.prepareStatement(sql)) {
  ...
  public List<FoodItem> getDiscountedFoodItems() throws SQLException {
    String sql =
        "SELECT * FROM food_item WHERE is_for_donation = false AND discounted_price IS NOT NULL and status != 5";
    List<FoodItem> items = new ArrayList<>();
    try (Connection conn = DatabaseConnection.getConnection();
        PreparedStatement pstmt = conn.prepareStatement(sql)) {
      try (ResultSet rs = pstmt.executeQuery()) {
