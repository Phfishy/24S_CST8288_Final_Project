...
public class OderDao { // Create (Insert) a new order
    public Order createOrder(Order order) throws SQLException {
        String sql = "INSERT INTO `order` (item_id, buyer_id, quantity, checkout_time, transaction_type, status) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public boolean updateOrder(Order order) throws SQLException {
        String sql = "UPDATE `order` SET item_id = ?, buyer_id = ?, quantity = ?, checkout_time = ?, transaction_type = ?, status = ? WHERE id = ?";
